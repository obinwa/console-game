...
class Ship{
  ...
  constructor(alignment,startPosition, endPosition){
    ...
    if(!this.validateShipPosition()){ //(alignment,startX, endX,startY,endY)){
      throw new Error("Ship position is invalid!")
    }
  }

  validateShipPosition() {// alignment, startPosition, endPosition){
  ...
  toString(){
    return `{alignment = ${this.#alignment}, start position = ${this.#startPosition.toString()}, end position = ${this.#endPosition.toString()} }`;
  }
